...
def find_hospital(special_m, lati, long):
    ...
    key = "NOyoX3P7nTh2Mkrq%2FbdWbn1GES1%2BSGk6BCzTi0m3L25lICnejVggRxaNhTMUR%2B5Fb2e6sdfw9pFQT%2Fn2YUc%2FIg%3D%3D"
...
with tab2:
    ...
    month_list_df = data[(data['월별'] >= min_week) & (data['월별'] <= max_week)]
...
## -------------------- ▼ 2-1그룹 금일 출동 이력 출력 ▼ --------------------
    ...
    today_count = data[data['출동일시']==today_date].count().sum()
    ...
    if select_bins == '일자별':
        ...
        group_week = data.groupby(by='주별', as_index=False)['ID'].count()
        ...
        group_month = data.groupby(by='월별', as_index=False)['ID'].count()
    ...
    with col231:
        ...
        st.line_chart(data=group_week_time,x='주별',y='이송 시간',use_container_width=True)
    ...
    with col232:
        ...
        st.line_chart(data=group_month_time,x='월별',y='이송 시간',use_container_width=True)
...
# ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ [도전 미션] ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ ▼ 
    ...
    with col240: # 일간 통계
        ...
        fig = px.pie(group_day_disease, values='count', names='중증질환', height=360, width=360)
    ...
    with col241: # 주간 통계
        ...
        group_week_disease = group_week_disease.rename(columns={'주별':'count'})
    ...
    with col242: # 월간 통계
        ...
        group_month_disease = group_month_disease.rename(columns={'월별':'count'})
